...
for p in power:
    ...
    path = r"G:\Shared drives\LFL\Projects\Quasiparticles\NBR19_testJun10_2022_2\{}mK\Current_{}mA\{}\\".format(T,Vcurrent,StringForFlux)
    figpath = r"G:\Shared drives\LFL\Projects\Quasiparticles\NBR19_testJun10_2022_2\{}mK\Current_{}mA\figures\\".format(T,Vcurrent)
    ...
    data = qp.GaussianConvolution(data, 0.5, 10)
    data = qp.uint16_to_mV(data)
    ax = qp.plotComplexHist(data[0],data[1])
    ax.set_title(f'{p} dB')
    plt.savefig(figpath+f'\\{p}_dB.png')
    plt.show();
    plt.close();
