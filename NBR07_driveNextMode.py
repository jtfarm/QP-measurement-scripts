...
ph = 0.45
nmf = 12.8328e9
estimated_freq = 4.27409*1e9
fd = 4.273882
shift = estimated_freq - fd*1e9

SPATH = r"E:\\driveNextMode_LongerTraces_{:3.0f}\\".format(ph*1000)
...
powersetting = np.round(np.arange(-25,10,1),decimals=0)
attens = np.round(np.arange(50,0,-1),decimals=0)
...
lfVNA = Labber.createLogFile_ForData('NBR07_driveNextMode_LongerTraces_{:3.0f}'.format(ph*1000),
                                      [{'name':'VNA - S21','complex':True,'vector':True,'x_name':'Frequency','x_unit':'Hz'}],
                                      step_channels=[{'name':'Power','values':powers,'unit':'dBm'}])

I = curFunc(ph)

# set up the DA and sig gen
LO2.setValue('Power',-25)
DA.setValue('Attenuation',50)
...
for count,p in enumerate(powers):
    ...
    LO.setValue('Frequency',fcenter-shift)
    ...
    StringForFlux = r'{:3.0f}dBm\SR{:d}MHz'.format(p*10,sampleRateMHz)
