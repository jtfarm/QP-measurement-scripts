...
SPATH = r"E:\\MonitorDuringCoolown\\"
...
interval = 30 #seconds
totalDuration = 24*3600 #24 hours in seconds
NPOINTS = totalDuration//interval
TIMES = np.arange(NPOINTS)*interval
...
lfVNA = Labber.createLogFile_ForData('NBR07_MonitorDuringCooldown',
                                      [{'name':'VNA - S21','complex':True,'vector':True,'x_name':'Frequency','x_unit':'Hz'}],
                                      step_channels=[{'name':'Time','values':TIMES,'unit':'s'}])

fluxSetting = 0.47

estimated_freq = 4.269*1e9
I = curFunc(fluxSetting) # mA
logging.info(f'\n\nStarting phi = {fluxSetting:.3f}.\nEstimated frequency is {estimated_freq*1e-9:.6f} GHz.\nCurrent on coil is {I:.6f} mA.')
...
shiftGuess = 4e-4
for count,ti in enumerate(TIMES):
    now = perf_counter()
    ...
    VNA.setValue('Output enabled',True)
    VNA.setValue('Output power',-30)
    VNA.setValue('# of averages',300)
    sleep(0.1)
    VNA.setValue('Trigger',True)
    ...
    try:
        ...
        fguess = X[fguessInd]
        shiftGuess = 1.5e-9*(qp.f_n_phi(fluxSetting,0)-qp.f_n_phi(fluxSetting,1))
        pars,cov = curve_fit(sumLor,X,zData.real,p0 = [0.8,0.5,0.1,fguess,shiftGuess,0.00025],bounds=([0.1,0.002,1e-6,fguess-500e-6,shiftGuess-0.1*shiftGuess,0.0002],[2,2,2,fguess+500e-6,shiftGuess+0.1*shiftGuess,0.00028]))
        ...
        plt.legend()
        plt.title(f'Time = {ti:.1f} | shift = {pars[4]*1e6:.1f} kHz')
        plt.ylabel('S21 - real')
        plt.xlabel('Frequency [GHz]')
        plt.savefig(figpath+f'TIME_{int(ti)}.png')
        plt.show()
        plt.close()
        f0 = pars[3]
        fd = pars[3]-pars[4]
        ...
        try:
            ...
            plt.axvline(pars[1])
            plt.legend()
            resshift = (qp.f_n_phi(fluxSetting,0)-qp.f_n_phi(fluxSetting,1))
            f0 = pars[1]
            fd = pars[1] - 1.5e-9*resshift
            pars[2] = fd
            plt.axvline(fd)
            plt.title(f'Time = {ti:.1f} | shift = {resshift*1e-3:.1f} kHz')
            plt.ylabel('S21 - real')
            plt.xlabel('Frequency [GHz]')
            plt.savefig(figpath+f'TIME_{int(ti)}.png')
            plt.show()
    ...
    with open(figpath+f'TIME_{int(ti)}_fit.pkl','wb') as pkfile:
    ...
    logging.info(f'\nStarting Alazar acquisition while driving at {fd:.6f} GHz')
    DAsetting = 15
    DA.setValue('Attenuation',DAsetting)
    StringForFlux = r'{:3.0f}flux\DA{:2.0f}_SR{:d}MHz'.format(fluxSetting*1000,DAsetting,sampleRateMHz)
    ...
    print(f'There are {Nremain} steps cycles left, estimated time remaining = {interval*Nremain/3600:.3f} hours')
    ...
    sleeptime = interval - (perf_counter()-now)
    if sleeptime > 0:
        sleep(sleeptime)
    else:
        print(f'Missed the clock: {sleeptime}')
        logging.info(f'Missed the clock: {sleeptime}')