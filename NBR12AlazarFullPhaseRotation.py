...
# LO.startInstrument()
# SMU = client.connectToInstrument('Keithley 2400 SourceMeter',dict(interface='GPIB',address='18'))
# SMU.startInstrument()
...
# lI = np.arange(3.1e-3,3.3e-3,0.005e-3)
x = []
y = []
i = 0
while True:
    ...
    path = r"G:\Shared drives\LFL\Projects\Quasiparticles\NBR19_Jun10_2022\fullPhaseRotation_noJPA\{}\\".format(StringForFlux)
    figpath = r"G:\Shared drives\LFL\Projects\Quasiparticles\NBR19_Jun10_2022\fullPhaseRotation_noJPA\Figures\\"
    ...
    # ax.set_title(f'{I*1e3:.1f} mA')
    # v, w = np.linalg.eigh(np.cov(data))
    # varis = np.sqrt(v)y
    ax.set_aspect('equal')
    plt.savefig(figpath+f'\\{i}.png')
    plt.close()
    print(varis)
    i += 1
